 ...
 * # Contract
 *
 * Either [parse] succeeds returning [Success] or fails returning an [Error], in which case it must
...
abstract class Parser (vararg val children: Parser)
{
    companion object {
        /**
         * Use this builder to create subclasses of [Parser].
         *
    ...
    /**
     * Builds an error whose message will be generated by [msg].
     * It is advised to use [Context.error] instead.
     *
     * If [Context.debug] is true, this generates instances of [DebugError], giving more info at
     * the cost of performance.
     */
    fun Context.plainError(msg: Parser.(Context) -> String): Error {
        val msg2 = { this@Parser.msg(this) }
        return if (!debug) Error(pos, msg2)
        else DebugError(pos, msg2, StackTrace(), trace.link, snapshot())
    }

    /**
     * Builds an error (through [plainError]) whose message will be generated by [msg] and
     * annotated with the name of the parser and the current input input position.
     */
    fun Context.error(msg: Parser.(Context) -> String): Error {
        return plainError { this@Parser.msg(this@error) + " (in ${toStringSimple()} at $posStr)" }
    }

    /**
     * Builds an error (see [plainError]) with a default error message including the name of this
     * parser and the current input position.
     */
    fun Context.error()
        = plainError { "in ${toStringSimple()} at $posStr" }

    /**
     * Panic, throwing the given error, which can be caught with [tryParse] or
     * [norswap.autumn.parsers.Chill], or will be caught by [parse] as a last resort.
     */
    fun panic(e: Error): Result = throw Carrier(e)

    /// Utilities ----------------------------------------------------------------------------------

    /**
     * Calls [f] after making a snapshot.
     * If the result is [Error], restores the snapshot.
     * Returns the result of [f].
     */
    inline fun Context.transact(f: () -> Result): Result {
        val snapshot = snapshot()
        return f().apply { if (this is Error) restore(snapshot) }
    }

    /**
     * Shorthand for `if (cond()) Success else error()` (see [error]).
     */
